...
<html lang="es">
    <head>
        ...
        <title>Asignación de Reservas | Estetica</title>
        <!-- Bootstrap core CSS -->
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="x_panel" style="">
                                    <div class="x_title">
                                        <h2 id="titulo">Asignacion de Reservas a sucursal</h2>
                                        <div class="clearfix"></div>
                                    </div>
                                            ...
                                            <div id="form" class="form-horizontal animated bounce form-label-left">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-9 col-sm-9 col-xs-12 xdisplay_inputx form-group has-feedback">
                                                        ...
                                                        <select required="required" class="form-control has-feedback-left" id="nombre-sucursal" ></select>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-9 col-sm-9 col-xs-12">
                                                        <input type="hidden" id="id" class="form-control has-feedback-left" required="required">
                                                        <input id="fecha" class="form-control has-feedback-left">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-9 col-sm-9 col-xs-12">
                                                        <select id="horario" required="" class="form-control has-feedback-left"></select>
                                                        <span class="fa fa-calendar-o form-control-feedback left" aria-hidden="true"></span>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
                                                        <button id="enviar" class="btn btn-success">Seleccionar cliente</button>
                                                    </div>
                                                </div>
                                            </div>
                                    </form>
                                    <table id="tabla-Reservas" class="table table-striped table-bordered">
                                        <thead>
                                            <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        tablaTrabajadores = $('#tabla-Reservas').DataTable({
            dom: 'Bfrtip',
            buttons: [
                {
                    extend: 'excelHtml5',
    ...
    $('#nombre-producto').change(function () {
        listarTrabajadores($(this).val());
    });
    ...
    function registrarReserva(cliente) {
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {funcion: 'registrarReserva', idsucursalproducto: $("#nombre-producto option:selected").data('idsucursalproducto'), idcliente: cliente, idtrabajador: $('#nombre-trabajador').val(), fecha: $("#fecha").val(), idhorario: $("#horario").val()},
                ...
                if (pS.status === 200) {
                    $.each(datos, function (i, trabajador) {
                        ...
                        tablaTrabajadores.row.add([
                            ...
                            trabajador.cliente,
                            trabajador.fecha,
                            '<button type="button"  data-id="' + trabajador.id + '"  class="eliminar-notificacion btn btn-danger">Eliminar</button>']).draw(false);
                    });
                    $(".eliminar-notificacion").click(function () {
                        eliminarReserva($(this).data('id'));
                    });
            ...
            success: function (datos, x, pS) {
                ...
                if (pS.status === 200) {
                    $.each(datos, function (i, producto) {
                        $("#nombre-producto").append('<option value="' + producto.idproducto + '" data-idsucursalProducto="' + producto.id + '" >' + producto.nombreProducto + '</option>');
                    });
            ...
            success: function (datos, x, pS) {
                ...
                if (pS.status === 200) {
                    $.each(datos, function (i, persona) {
                        tablaPersonas.row.add([persona.nombre, persona.apellido, persona.email, persona.telefono, '<button type="button" data-id="' + persona.id + '" class="reservar-cupo btn btn-primary">Reservar</button>', '<button type="button"  data-id="' + persona.id + '"  class="eliminar-notificacion btn btn-danger">Eliminar</button>']).draw(false);
                    });
                    $(".reservar-cupo").click(function () {
                        registrarReserva($(this).data('id'));
                    });
    ...
    function listarTrabajadores(id) {
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
            data: {funcion: 'listarTrabajadores', idproducto: id},
            success: function (datos, x, pS) {
                if (pS.status === 204) {
                    notify('Notificación', 'No se encontraron Reservas en esta sucursal', 'warning');
                }
    ...
    function listarHorarios() {
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {funcion: 'listarHorarios', idpersona: $("#nombre-trabajador").val(), fecha: $("#fecha").val()},
            success: function (datos, x, pS) {
                if (pS.status === 204) {
                    notify('Notificación', 'No se encontraron horarios disponibles en esta fecha', 'warning');
                }
                $("#horario").html('<option value="">--Seleccione--</option>');
                if (pS.status === 200) {
                    $.each(datos, function (i, horario) {
                        $("#horario").append('<option value="' + horario.id + '">' + horario.hora_inicio + ' - ' + horario.hora_fin + '</option>');
                    });
    ...
    function eliminarReserva(id) {
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {funcion: 'eliminarReserva', id: id},
            success: function (datos, x, pS) {
                if (pS.status === 204) {
                    notify('Notificación', 'No se encontraron registros', 'warning');
                }
...
<div  id="lista-personas" class="modal  fade" tabindex="-1" role="dialog">
