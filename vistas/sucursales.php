...
<html lang="es">
    <head>
        ...
        <title>Sucursales | Estetica</title>
        <!-- Bootstrap core CSS -->
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="x_panel" style="">
                                    <div class="x_title">
                                        <h2 id="titulo">Sucursal</h2>
                                        <div class="clearfix"></div>
                                    </div>
                                    <form id="formulario-sucursal" action="" method="post">
                                        <div class="x_content">
                                            <div id="form" class="form-horizontal animated bounce form-label-left">
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-9 col-sm-9 col-xs-12">
                                                        <input type="hidden" id="id" class="form-control has-feedback-left" >
                                                        <input type="text" id="nombre" class="form-control has-feedback-left" required="required"placeholder="Nombre del sucursal">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-md-9 col-sm-9 col-xs-12 xdisplay_inputx form-group has-feedback">
                                                        <span class="fa fa-keyboard-o form-control-feedback left" aria-hidden="true"></span>
                                                        <textarea type="text" class="form-control has-feedback-left" required="required" id="direccion" placeholder="Direccón"></textarea>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label col-md-3 col-sm-3 col-xs-12">Teléfono</label>
                                                    <div class="col-md-9 col-sm-9 col-xs-12 xdisplay_inputx form-group has-feedback">
                                                        <input type="tel" min="0" class="form-control has-feedback-left" required="required" id="telefono" placeholder="Teléfono">
                                    ...
                                    <table id="tabla-sucursales" class="table table-striped table-bordered">
                                        <thead>
                                            <tr>
                                                <th>Nombre</th>
                                                <th>Dirección</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        tablaSucursales = $('#tabla-sucursales').DataTable({
            dom: 'Bfrtip',
            buttons: [
                {
                    extend: 'excelHtml5',
    ...
    $("#formulario-sucursal").submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
            data: {funcion: 'registrarSucursal', id: $("#id").val(), nombre: $("#nombre").val(), direccion: $('#direccion').val(), telefono: $("#telefono").val()},
    ...
    function listarSucursales() {
        $.ajax({
            ...
            success: function (datos, x, pS) {
                console.log(datos, x, pS)
                if (pS.status === 204) {
                    notify('Notificación', 'No se encontraron registros', 'warning');
                }
                tablaSucursales.row().clear().draw(false);
                if (pS.status === 200) {
                    $.each(datos, function (i, sucursal) {
                        console.log(sucursal)
                        tablaSucursales.row.add([sucursal.nombre, sucursal.direccion, sucursal.telefono, '<button type="button" data-id="' + sucursal.id + '" class="actualizar-notificacion btn btn-primary">Actualizar</button>', '<button type="button"  data-id="' + sucursal.id + '"  class="eliminar-notificacion btn btn-danger">Eliminar</button>']).draw(false);
                    });
                    $(".eliminar-notificacion").click(function () {
                        eliminarSucursal($(this).data('id'));
                    });

                    $(".actualizar-notificacion").click(function () {
                        seleccionarSucursal($(this).data('id'));
                    });
                }
            },
            error: function (e) {
    ...
    function eliminarSucursal(id) {
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {funcion: 'eliminarSucursal', id: id},
            success: function (datos, x, pS) {
                if (pS.status === 204) {
                    notify('Notificación', 'No se encontraron registros', 'warning');
                }
    ...
    function seleccionarSucursal(id) {
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {funcion: 'seleccionarSucursal', id: id},
            success: function (datos, x, pS) {
                if (pS.status === 204) {
                    notify('Notificación', 'No se encontraron registros', 'warning');
                }
